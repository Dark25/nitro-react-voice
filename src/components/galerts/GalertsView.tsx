...
export const GalertsView: FC<{}> = props =>
{
    ...
    function getHabboImaging(look){
        return "https://habbo-imaging.hgalaxy.net/avatarimage/" + look + "&gesture=sml&direction=2&effect=6&img_format=gif";
    }

    function goToRoom(){
        fetch("https://int.hgalaxy.fun/?type=goToRoom&roomId=" + roomId + "&sso=" + sso)
        setIsVisible(false)
    }
            ...
            linkReceived: (url: string) =>
            {
                ...
                switch(parts[1])
                {
                    case '@gm':
                        ...
                        fetch("https://int.hgalaxy.fun/?type=getFigure&getFigure=" + parts[3])
                        .then(response => response.text())
                        .then((response) => { 
                            setFigure(response);
                            setIsVisible(true);
                            var audio = new Audio('https://swfs.hgalaxy.net/sounds/ding.mp3');
                            audio.play();
                        })
                        return;
