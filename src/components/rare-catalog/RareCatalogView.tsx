...
export const RareCatalogView : FC<{}> = props =>
{
    ...
    function searchItem(){
        fetch("https://hgalaxy.net/system/controllers/eha.php?getItemsByName=" + busquedaString)
    ...
    useEffect(() =>
    {
        const linkTracker: ILinkEventTracker = {
            linkReceived: (url: string) =>
            {
                ...
                {
                    case 'show':
                        fetch("https://hgalaxy.net/system/controllers/eha.php?getLastItems=true")
                        .then(response => response.json())
                        .then((response) => { 
                            setLastRares(response);
                        })

                        fetch("https://hgalaxy.net/system/controllers/eha.php?getLastItemsCategory=1")
                        .then(response => response.json())
                        .then((response) => { 
                            setCustom(response);
                        })

                        fetch("https://hgalaxy.net/system/controllers/eha.php?getLastItemsCategory=2")
                        .then(response => response.json())
                        .then((response) => { 
                            setRares(response);
                        })

                        fetch("https://hgalaxy.net/system/controllers/eha.php?getLastItemsCategory=3")
                        .then(response => response.json())
                        .then((response) => { 
                            setMegas(response);
                        })

                        fetch("https://hgalaxy.net/system/controllers/eha.php?getLastItemsCategory=4")
                        .then(response => response.json())
                        .then((response) => { 
                            setUltras(response);
                        })
                                ...
                                { lastRares.map((rare) => 
                                    <div className="col-md-3">
                                        <div className="text-center" style={{backgroundImage: "url(https://files.habboemotion.com/resources/images/homebgs/bg_colour_07.gif)", padding: "10px", borderRadius: "3px", marginBottom: "10px"}}>
                                            ...
                                            <span className="badge bg-dark"><img src="https://swfs.akiled.org/nitro/images/wallet/0.png" /> {rare.price_diamonds}</span><br/>
                                            <span className="badge bg-primary"><img src="https://swfs.akiled.org/nitro/images/wallet/5.png" /> {rare.price_th}</span>
                                        </div>
                                    </div>
                                )}
                                </div>
