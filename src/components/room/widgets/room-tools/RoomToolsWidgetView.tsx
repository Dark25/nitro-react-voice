...
export const RoomToolsWidgetView: FC<{}> = props =>
{
    ...
    return (
        <Flex className="nitro-room-tools-container" gap={ 2 } style={{marginBottom: "45px"}}>
            <Column center className="nitro-room-tools p-2">
                ...
                <Base pointer title={ LocalizeText('room.chathistory.button.text') } onClick={ e => openBawTool() } className="icon icon-baw-tool" />
                <Base pointer title={ LocalizeText('room.chathistory.button.text') } onClick={ event => CreateLinkEvent('ratecatalog/show') } className="icon icon-ltd-catalog" />
                ...
                <TransitionAnimation type={ TransitionAnimationTypes.SLIDE_LEFT } inProp={ isOpen } timeout={ 300 }>
                    <Column center>
                        <Column className="nitro-room-tools-info rounded py-2 px-3">
                            <Column gap={ 1 }>
                                <Text wrap variant="none" style={{color: "var(--test-galaxytext)"}} fontSize={ 4 }>{ roomName }</Text>
                                <Text variant="none" style={{color: "var(--test-galaxytext)"}} fontSize={ 5 }>{ roomOwner }</Text>
                            </Column>
