            ...
            <Flex alignItems="center" justifyContent="between" gap={ 3 } className="nitro-toolbar py-1 px-3 animate__animated animate__backInUp animate__faster" style={{marginBottom: "17px"}}>
                <Flex gap={ 2 } alignItems="center">
                    <Flex alignItems="center" gap={ 2 } style={{ marginBottom: "3px",paddingTop: 2, paddingBottom: 2, paddingLeft: 10, paddingRight: 10}}>
                        ...
                        { isInRoom &&
                            <Base style={{borderRadius: "16px", backgroundColor: "var(--test-galaxy)", padding: "25px"}} pointer className="navigation-item icon icon-habbo icon-border message-icon-show" onClick={ event => VisitDesktop() } /> }
                        { !isInRoom &&
                            <Base style={{borderRadius: "16px", backgroundColor: "var(--test-galaxy)", padding: "25px"}} pointer className="navigation-item icon icon-house icon-border" onClick={ event => CreateLinkEvent('navigator/goto/home') } /> }
                        <Base style={{borderRadius: "16px", backgroundColor: "var(--test-galaxy)", padding: "25px"}} pointer className="navigation-item icon icon-rooms icon-border" onClick={ event => CreateLinkEvent('navigator/toggle') } />
                        <Base style={{borderRadius: "16px", backgroundColor: "var(--test-galaxy)", padding: "25px"}} pointer className="navigation-item icon icon-catalog icon-border" onClick={ event => CreateLinkEvent('catalog/toggle') } />
                        <Base style={{borderRadius: "16px", backgroundColor: "var(--test-galaxy)", padding: "25px"}} pointer className="navigation-item icon icon-inventory icon-border" onClick={ event => CreateLinkEvent('inventory/toggle') }>
                            { (getFullCount > 0) &&
